...
userActionEmulation$.pipe(
  ...
  switchMap(sourceValue => sucessRequestEmulation$.pipe(map(x => ({ value: sourceValue.value, response: x.value })))),
  switchMap(sourceValue => errorRequestEmulation$.pipe(catchError(error => of(error)), map(x => { sourceValue.error = x.error; return sourceValue }))),
  map(value => `Value - ${value?.value}; Response - ${value?.response}; Error - ${value?.error}`)
  // YOUR CODE ENDS HERE
